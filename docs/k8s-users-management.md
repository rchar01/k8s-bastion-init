...
The daemon writes only temporary `~/.kube/bootstrap`. Final `~/.kube/config`, `~/.kube/user.crt`, and `~/.kube/user.key` are written by user-side enrollment flow.
