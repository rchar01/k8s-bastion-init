...
5. Login auto-bootstrap (`bastion-login-bootstrap`) recovers missing/expired/broken credentials through local root daemon `bastion-bootstrapd`; renewal timer and `bastion-renew-cert` handle in-band renewals.
